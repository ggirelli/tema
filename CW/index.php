...
<head>
  ...
  $(document).ready(function() {
          ...
          if(graphn != $('#hidden-graphn').val()) {
            if(graphn > $('#hidden-graphn').val()) {
              ...
              for (var i = 0; i < graphn - $('#hidden-graphn').val(); i++) {
                $("<input type='file' class='graph-upload' id='graph-upload-" + (parseInt($('#hidden-graphn').val()) + i) + "' name='graph-upload-" + (parseInt($('#hidden-graphn').val()) + i) + "' />").appendTo($('#form'));
              }
            } else {
    ...
    $('#form #graphn').bind("keypress", function(e) {
      if (e.keyCode == 13) {               
        e.preventDefault();
...
<form id='form' class='col-md-6 col-md-offset-3' method='post' action='graph_loader.php' enctype="multipart/form-data">
