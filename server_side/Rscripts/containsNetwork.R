	...
	if(file.exists(paste0(args[2], '.json'))) {
		...
		if ( 0 != vcount(g) ) {
			...
			if ( 0 != ecount(g) ) {
				...
				# Add source/target
				if ( 'name' %in% list.vertex.attributes(g) ) {
					e_super_attr_table <- cbind(e_super_attr_table, n_super_attr_table[unlist(lapply(get.edgelist(g)[,1], FUN=function(x,g) {return(c(V(g)[name==x]))}, g)), 'sogi_node_identity'])
					colnames(e_super_attr_table)[ncol(e_super_attr_table)] <- 'source'
					e_super_attr_table <- cbind(e_super_attr_table, n_super_attr_table[unlist(lapply(get.edgelist(g)[,2], FUN=function(x,g) {return(c(V(g)[name==x]))}, g)), 'sogi_node_identity'])
					colnames(e_super_attr_table)[ncol(e_super_attr_table)] <- 'target'
				} else {
					e_super_attr_table <- cbind(e_super_attr_table, n_super_attr_table[unlist(lapply(get.edgelist(g)[,1], FUN=function(x,g) {return(c(V(g)[id==x]))}, g)), 'sogi_node_identity'])
					colnames(e_super_attr_table)[ncol(e_super_attr_table)] <- 'source'
					e_super_attr_table <- cbind(e_super_attr_table, n_super_attr_table[unlist(lapply(get.edgelist(g)[,2], FUN=function(x,g) {return(c(V(g)[id==x]))}, g)), 'sogi_node_identity'])
					colnames(e_super_attr_table)[ncol(e_super_attr_table)] <- 'target'
				}
