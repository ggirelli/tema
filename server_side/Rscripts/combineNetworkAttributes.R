...
# Check parameters
if(length(args) != 6) stop('./addAttributeToNetwork.R session_id graph_name attr_type attr_name attr_list attr_function')
...
if(file.exists(paste0('/home/gire/public_html/SOGIv020/server_side/session/', args[1], '/'))) {
	...
	if ( 'edges' == args[3] ) {
		...
		attr_list <- unlist(strsplit(args[5], ',', fixed=T))
		for (i in 1:length(attr_list)) {
			fun <- gsub(paste0('_', i-1, '_'), paste0('V(g)$', attr_list[i]), fun)
		}
