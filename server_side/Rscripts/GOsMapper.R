...
# Check parameters
if(length(args) != 1) stop('./addGOattributueToNetwork.R session_id')
...
# Start
if(file.exists(paste0('../session/', args[1], '/settings/'))) {
	setwd(paste0('../session/', args[1], '/settings/'))

	# BUILD GOA-MAPPING LIST #

	rows <- scan('goa.dat', 'raw', sep='\n')
